...
[Netlify Drop](https://app.netlify.com/drop) is a free publishing option that does not require a Netlify account to start, and does not rely on your **bookdown** project being in a GitHub repository. All you need for this approach is a **bookdown** project that you can build locally. Click [here](https://youtu.be/-LRlQ_jaLAU) to watch a quick video introduction to [Netlify Drop](https://app.netlify.com/drop).
...
Start with a **bookdown** project (it does not need to be in GitHub!). 
...
![Screenshot of drag & drop deploy box on Netlify](images/netlify-drop.png)
...
This workflow is identical to that described in \@ref(ghpages) using GitHub Project Pages. Using Netlify requires an additional service, but that addition does not add any complexity in terms of accounts (if you sign up using your GitHub account) or file structures present in your repository. These two approaches are basically the same, and in fact you can host the same book source files on *both* a GitHub Project Page and a Netlify site. 
...
1. You are satisfied using (the default) project domain name like `https://<USER>.github.io/<REPO>`.^[Note that you can use custom domains with GitHub Pages, see: https://help.github.com/articles/using-a-custom-domain-with-github-pages/.]
...
+ **Use Netlify** 
  ...
  + [Sign up for Netlify with GitHub](#netlify-new)
