...
You can host your book on GitHub\index{GitHub} for free via GitHub Pages (https://pages.github.com).^[See the [GitHub Pages Help documents](http://bit.ly/2cvloKV) for more information.] The simplest approach is to publish your book as a GitHub Project Page from a `/docs` folder on your `master` branch, which will then be available at `https://<USER>.github.io/<REPO>`. The main benefit to this approach is its simplicity, and the fact that you can track the source files for your book and the published HTML files in the same branch.

### The build-and-deploy pipeline sequence

This publishing approach sets up the following flow of events:

1. You start with a local **bookdown** project in a GIT repository that is connected to a GitHub repository. The below steps assume you are on your `master` branch.^[If you are not familiar with GitHub, we suggest you read through the free online book ["Happy Git with R"](https://happygitwithr.com/) first before working through these instructions.]
...
1. The push to `master` triggers GitHub Pages to publish your book at: `https://<USER>.github.io/<REPO>`.
1. Rinse and repeat! Every push to `master` triggers the online version of your book to update.


### Before you begin

Start with a **bookdown** project linked to a remote GitHub repository that you can push/pull to from your local copy. 
...
+ [Change anything you want and commit](https://happygitwithr.com/new-github-first.html#make-local-changes-save-commit-1),
...
In your local **bookdown** project, find the `_bookdown.yml` and set the output directory of your book to be `/docs` by adding this line:
...
This file is necessary because GitHub Pages supports Jekyll ([jekyllrb.com](http://jekyllrb.com)), a static website builder, by default. Because the **bookdown** HTML output is a static, standalone website, you need to tell GitHub that your website is *not* to be built via Jekyll. 

### Build your book and push to GitHub

Use whichever method from Chapter \@ref(build-the-book) you prefer. 
...
### Activate your GitHub Pages site

1. Go to your repository on GitHub at `https://github.com/<USER>/<REPO>` and confirm that you see a `/docs` folder and a `.nojekyll` file. 
...
1. You are satisfied using a project domain name like `https://<USER>.github.io/<REPO>`. *Note that you can use custom domains with GitHub Pages, described [here](https://help.github.com/articles/using-a-custom-domain-with-github-pages/).*
...
If you do not already have a Netlify account and you do have a GitHub account, we suggest you when you "Sign Up" and you see the options "Sign up with one of the following:" that you select `GitHub` (Netlify also currently supports GIT repositories hosted on GitLab and BitBucket as well).
...
Here are some steps you may follow to setup your **bookdown** project for continuous deployment through Netlify:
...
In this section, we'll describe how to use Travis to automate the build and deployment step, deploying either to [GitHub Pages](travis-ghpages) or to Netlify. Travis CI\index{Travis CI} ([travis-ci.org](https://travis-ci.org)) is a service that builds and checks your software in a virtual machine whenever you push changes to GitHub. It is free to use for public repositories on GitHub, and was designed for continuous integration (CI) of software packages. 
...
The major benefit to using Travis within a **bookdown** project is that you are able to make changes to your book source files without needing to locally render the book. Any change to any `.Rmd` file in your book that is pushed to GitHub will set this build-deploy pipeline in motion, even if you do not or cannot build the book locally. This may be useful both when collaborating with yourself or with other people. 
...
### 01: Deploy from branch {#travis-branch}

This is a good workflow to start with if you are newer to using Travis. We'll have Travis push the built book to the `gh-pages` branch, and then we will deploy the **bookdown** site from that branch (instead of the `master` branch). We'll use either GitHub Pages or Netlify to publish the site- the workflows are very similar. You will never need to directly push any changes to the `gh-pages` branch- this is a branch that only Travis will push to. Deploys from the `gh-pages` branch **only** should be be published automatically. After the initial setup, every time you push to your master branch, this will trigger your book to be (re-)built on the `gh-pages` branch, which will in turn trigger a fresh deploy. 

The major benefit to this approach is that your master branch on GitHub will no longer store the rendered book files (i.e., the folders `_book` and `_bookdown_files`). Instead, those files will be present in a separate branch: the `gh-pages` branch. This means that your master branch will contain source files *only*.

A second benefit here is that you will *not* experience a double-deployment problem. This would happen if for example you triggered a new Travis build with a push from your `master` branch, *and* deployed from the same branch. What you would see then is that your **bookdown** site would go down temporarily after you push to GitHub, because no book would be there to publish! So, this workflow is nice as deploys are only triggered once your book is built by Travis. In more advanced workflows using Netlify that we discuss later, we will show ways to keep Netlify from trying to build on every push. But for this reason, we encourage you start here with Travis and **bookdown**.
...
+ In addition to the `DESCRIPTION`, you also need to tell Travis how to build the book in a new file called `.travis.yml`. You can create this file in the terminal with:
  ...
  An easy way to get into a terminal from RStudio is *Tools > Terminal*. If you are on Windows, you can create the file in R using `file.create(".travis.yml")`. We'll provide a complete example of a `.travis.yml` file here, then walk through each of the sections step-by-step below:
...
#### 01a: To GitHub Pages {#travis-ghpages}

every time you push to your master branch, this will trigger your book to be (re-)built, which will in turn trigger a fresh deploy to your GitHub Pages site at `https://<USER>.github.io/<REPO>`.
...
### 02: Netlify from branch {#netlify-branch}
...
+ Very important! Your GitHub repo must have at least a README file in it; it cannot be empty.
...
+ You may wish to back up your local copies of the `_book` and `_bookdown_files` first (unless you have modified your `output_dir` in your `_bookdown.yml` file), but you can safely delete and ignore both of these folders- these will now be built by Travis. An easy way to edit the `.gitignore` file is to use the **usethis** package (https://usethis.r-lib.org):
    ```{r ref.label="usegitignore", eval = FALSE}

    ```


If your book was created as part of an R Project, your updated `.gitignore` would then look like this:
